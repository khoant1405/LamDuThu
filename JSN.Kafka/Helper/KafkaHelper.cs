...
namespace JSN.Kafka.Helper
{
    public sealed class KafkaHelper
    {
        ...
        private static class Nested
        {
            ...
            internal static readonly KafkaHelper instance = new KafkaHelper();
        }
        private KafkaHelper()
        {
        }
        ...
        private KafkaSetting _kafka;
        private Metadata _kafkaMetadata;
        private IProducer<string, string> _producer;
        #endregion

        #region Set Kakfa config
        public void SetKafkaConfig(KafkaSetting kafka)
        {
            _kafka = kafka;
            // consumer config
        ...
        public void InitProducer()
        {
            ...
            {
                KvException.WriteException(new LogObject(GetType().Name, $"{GetType().Name}.{nameof(InitProducer)}"), new KafkaException(e), null, $"Kafka Producer Error: {JsonConvert.SerializeObject(e)}");
            });
            // set log handle
            if (KvAppConfig.IsKafkaMonitor)
            {
            ...
            consumer.SetErrorHandler((_, e) =>
                {
                    ...
                    KvException.WriteException(new LogObject(GetType().Name, $"{GetType().Name}.SetErrorHandler"), new KafkaException(e), null, $"Kafka Consumer Error: {JsonConvert.SerializeObject(e)}");
                })
                .SetLogHandler((_, e) =>
                {
                ...
                }).SetPartitionsAssignedHandler((c, partitions) =>
                {
                    ...
                    {
                        KvException.WriteInfo($"Assigned partitions: IndexConsumer: {indexConsumer} | Topic: {partitions.FirstOrDefault()?.Topic} | Partitions: [{string.Join(", ", partitions.Select(p => p.Partition.Value).ToList())}]", KvLogType.Kafka);
                    }
                })
