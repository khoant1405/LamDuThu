...
namespace JSN.Shared.Utilities
{
    public static class ConvertHelper
    {
        ...
        {
            return float.TryParse(obj?.ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out var parsedVal) ? parsedVal : defaultValue;
        }

        public static double ToDouble(object obj, double defaultValue = 0)
        {
            return double.TryParse(obj?.ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out var parsedVal) ? parsedVal : defaultValue;
        }

        public static decimal ToDecimal(object obj, decimal defaultValue = 0)
        {
            return decimal.TryParse(obj?.ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out var parsedVal) ? parsedVal : defaultValue;
        }
